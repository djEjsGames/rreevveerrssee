...
local ctx = {
	...
	player = { x = 64, y = 64, r = 12, aim = 0, profile = 1, dash_cd = 0, dash = nil, groggy = 0, sight_bonus = 0, fov_bonus = 0, speed_bonus = 0 },
...
function love.update(dt)
	...
	ctx.player.aim = atan2(wy - ctx.player.y, wx - ctx.player.x)
end
...
function love.keypressed(key)
	...
	if key == "n" then
		ui.trigger_minimap_interference(ctx, { mode = "diag_flip", flip_xy = true })
	end
...
function love.draw()
	...
	for y = 1, ctx.MAP_H do
		for x = 1, ctx.MAP_W do
			if ctx.map[y][x] == 1 then
				...
				love.graphics.rectangle("fill", (x - 1) * ctx.TILE, (y - 1) * ctx.TILE, ctx.TILE, ctx.TILE)
			end
		end
	end
	...
	for _, mob in ipairs(ctx.mobs) do
		if mob.alpha > 0.02 then
			draw_upright_at(mob.x, mob.y, function()
				love.graphics.setColor(mob.color[1], mob.color[2], mob.color[3], mob.alpha)
				love.graphics.circle("fill", mob.x, mob.y, 11)
				love.graphics.setColor(0.12, 0.08, 0.04, mob.alpha)
				love.graphics.circle("fill", mob.x - 4, mob.y - 2, 2)
				love.graphics.circle("fill", mob.x + 4, mob.y - 2, 2)
			end)
		end
	end
	...
	if ctx.seija and ctx.seija.alpha > 0.02 then
		...
		draw_upright_at(ctx.seija.x, ctx.seija.y, function()
			...
			love.graphics.circle("fill", ctx.seija.x, ctx.seija.y, ctx.seija.r)
			love.graphics.setColor(0.18, 0.08, 0.22, ctx.seija.alpha)
			love.graphics.circle("fill", ctx.seija.x - 4, ctx.seija.y - 2, 2)
			love.graphics.circle("fill", ctx.seija.x + 4, ctx.seija.y - 2, 2)
		end)
	end
	...
	draw_upright_at(ctx.player.x, ctx.player.y, function()
		love.graphics.setColor(1, 0.95, 0.55, 0.7)
		love.graphics.line(ctx.player.x, ctx.player.y, ctx.player.x + math.cos(ctx.player.aim) * 34, ctx.player.y + math.sin(ctx.player.aim) * 34)
		love.graphics.setColor(0.35, 0.72, 0.95)
		love.graphics.circle("fill", ctx.player.x, ctx.player.y, ctx.player.r)
